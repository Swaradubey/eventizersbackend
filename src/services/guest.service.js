...
const findGuestsByUserId = async (userId, search = "", eventId = "", page = null, limit = null, group = "") => {
  ...
  let query = `
    SELECT 
      ...
      g.rsvp_status AS "rsvp_status",
      COALESCE(g.adults_count, 1) AS "adultsCount",
      COALESCE(g.children_count, 0) AS "childrenCount",
      COALESCE(g.additional_guests, 0) AS "additionalGuests",
      g.dietary_preference AS "dietaryPreference",
      g.special_dietary_requests AS "specialDietaryRequests",
      g.responded_at AS "respondedAt",
      g.is_opened AS "isOpened",
      g.is_clicked AS "isClicked",
      g.opened_at AS "openedAt",
      g.clicked_at AS "clickedAt",
